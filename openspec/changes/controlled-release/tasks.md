...
- [ ] 4.1 Re-trigger the release event: v0.1.0 was created but the `release: types: [published]` workflow event did not fire — `:stable` was never built. Delete and recreate v0.1.0 (or create v0.1.1) to trigger the build.
...
- [ ] 5.1 Push updated `compose.prod.yml` to each Pi and restart:
  ```bash
  ...
  ```
- [ ] 5.2 Verify container on each Pi is running `:stable`
