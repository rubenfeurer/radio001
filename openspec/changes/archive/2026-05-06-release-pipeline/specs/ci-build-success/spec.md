...
The Docker build process SHALL complete successfully without errors related to missing configuration files or dependencies, and SHALL produce a published image on GHCR when building on `main` or a release tag.
...
- **AND** the build SHALL use `requirements.lock` with hash verification
...
#### Scenario: Image published to GHCR after successful build
- **WHEN** the CI pipeline builds the image on `main` or a semver tag and the Trivy scan passes
- **THEN** the image SHALL be pushed to `ghcr.io/[owner]/radio001`
- **AND** the image SHALL be tagged with `latest` and the semver tag (if applicable)
