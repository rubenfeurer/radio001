...
#### Scenario: Station library mounted from config
- **WHEN** the backend container starts
- **THEN** the station library SHALL be mounted from `../config/stations.json` into the container
- **AND** no `stations.json` copy SHALL be required inside `backend/assets/`

#### Scenario: Sound files mounted from config
- **WHEN** the backend container starts
- **THEN** sound files SHALL be mounted from `../config/sounds/` into the container
- **AND** no sound file copies SHALL be required inside `backend/assets/sounds/`
