...
- **AND** run `docker compose pull` to pre-cache the image
- **AND** run `systemctl enable --now radio.service` as the sole container lifecycle starter
...
#### Scenario: Install does not start containers before systemd
- **WHEN** the install script prepares to start the radio service
- **THEN** it SHALL NOT call `docker compose up` directly before `systemctl enable --now`
- **AND** `systemctl enable --now radio.service` SHALL be the first command that creates and starts the containers
- **AND** no "container name already in use" error SHALL occur during installation
